 ...
 * @version $Id: tpl_main_page.php 4591 2006-09-23 04:25:15Z ajeh $
 */
?>
<body id="popupCouponHelp" onload="resize();">

<?php
  $couponID = (int)$_GET['cID'];
  $coupon = $db->Execute("select * from " . TABLE_COUPONS . " where coupon_id = '" . $couponID . "'");
  $coupon_desc = $db->Execute("select * from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $couponID . "' and language_id = '" . (int)$_SESSION['languages_id'] . "'");
  ...
  $get_result=$db->Execute("select * from " . TABLE_COUPON_RESTRICT . "  where coupon_id='".$couponID."' and category_id !='0'");
  $cats = '';
  ...
  $get_result=$db->Execute("select * from " . TABLE_COUPON_RESTRICT . "  where coupon_id='".$couponID."' and product_id !='0'");
