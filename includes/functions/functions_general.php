 ...
 * @version $Id: functions_general.php 5 2010-11-27 07:29:30Z kitsune3 $
 */
...
// Parse search string into indivual objects
  function zen_parse_search_string($search_str = '', &$objects) {
    $search_str = mb_convert_kana((strtolower($search_str)), "s");
...
////
  function zen_db_prepare_input($string) {
    if (is_string($string)) {
      return trim(zen_sanitize_string(stripslashes(mb_convert_kana($string))));
...
//    $str = mb_strcut($str, 0, $len, CHARSET);
 ...
 */
  function zen_get_country_zones($country_id) {
    ...
    $zones = $db->Execute("select zone_id, zone_name
                           ...
                           order by " .((((int)$country_id)=='107')?'zone_id':'zone_name'));
    while (!$zones->EOF) {
  ...
  function zen_js_zone_list($country, $form, $field) {
    ...
    while (!$countries->EOF) {
      ...
      $states = $db->Execute("select zone_name, zone_id
                              ...
                              order by " .(($countries->fields['zone_country_id']=='107')?'zone_id':'zone_name'));
      $num_state = 1;
      while (!$states->EOF) {
 ...
 */
  function replace_accents($s) {
    ...
    //Special thanks to Mr. Takahashi
    //$s = htmlentities($s);
