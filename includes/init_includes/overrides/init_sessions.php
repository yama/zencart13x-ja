 ...
 * @version $Id: init_sessions.php 3 2010-11-06 07:15:58Z kitsune3 $
 */
...
if (SESSION_USE_ROOT_COOKIE_PATH == 'True') {
  $path = ($request_type == 'SSL') ? DIR_WS_HTTPS_CATALOG : DIR_WS_CATALOG;
} else {
  $path = '/';
}
...
/**
 * set host_address once per session to reduce load on servers
 */
 ...
 */
if (SESSION_CHECK_USER_AGENT == 'True'  && !ereg('ser', $_SERVER['HTTP_USER_AGENT'])) {
  $http_user_agent = $_SERVER['HTTP_USER_AGENT'];
  if (!$_SESSION['SESSION_USER_AGENT']) {
    $_SESSION['SESSION_USER_AGENT'] = $http_user_agent;
  }
  if ($_SESSION['SESSION_USER_AGENT'] != $http_user_agent) {
     if (strstr($http_user_agent, 'Trident/4.0')) { //IE8 Fix
      if (strspn($http_user_agent, 'compatible; MSIE 7')) {//7 or 8?
        $http_user_agent = str_replace('MSIE 7', 'MSIE 8', $http_user_agent);
      } elseif (strspn($http_user_agent, 'compatible; MSIE 8')) {
        $http_user_agent = str_replace('MSIE 8', 'MSIE 7', $http_user_agent);
      }
      if ($http_user_agent != $_SESSION['SESSION_USER_AGENT']) {
         zen_session_destroy();
