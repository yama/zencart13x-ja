...
if (isset($_POST['action']) && (($_POST['action'] == 'process') || ($_POST['action'] == 'update'))) {
  ...
  $telephone = mb_convert_kana($telephone, "a");
  ...
  if (ADD_FURIGANA == 'true') {
    $kana_array = array(array('fieldName'=>'entry_firstname_kana', 'value'=>$firstname_kana, 'type'=>'string'),
    ...
    if ($_POST['action'] == 'update') {
      ...
      if ( (isset($_POST['primary']) && ($_POST['primary'] == 'on')) || ($_GET['edit'] == $_SESSION['customer_default_address_id']) ) {
        ...
        $sql_data_array[] = array('fieldName'=>'customers_telephone', 'value'=>$_POST['telephone'], 'type'=>'string');
        $sql_data_array[] = array('fieldName'=>'customers_fax', 'value'=>$_POST['fax'], 'type'=>'string');
...
//special thanks sumida
if (!isset($_GET['delete'])) {
  if (USE_COUNTRY_DROPDOWN == 'False' && SHOW_CREATE_ACCOUNT_DEFAULT_COUNTRY != $entry->fields['entry_country_id']) {
      $messageStack->add_session('addressbook', ERROR_ADDRESS_BOOK_COUNTRY_ERROR);
  }
}
