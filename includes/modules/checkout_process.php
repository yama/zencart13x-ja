...
if (REPETITION_CHECK_ENABLED == 'true') {
  if (isset($_SESSION['repetition_check']) && $_SESSION['repetition_check'] != '') {
  $repetition_check = $db->Execute("select count(*) as count
                                                from " . TABLE_REPETITION_CHECK . "
                                                where customers_id = '" . (int)$_SESSION['customer_id'] . "'
                                                and check_hash = '" . zen_db_prepare_input($_SESSION['repetition_check']) . "'
                                                ");
    if ($repetition_check->fields['count'] >= 1) {
          $messageStack->add_session('header', ERROR_REPETITION_ORDER, 'error');
          zen_redirect(zen_href_link(FILENAME_CHECKOUT_SUCCESS, '', 'SSL'));
          exit;
    } else {
      $repetition_array = array('customers_id' => (int)$_SESSION['customer_id'],
                                     'check_hash' => zen_db_prepare_input($_SESSION['repetition_check']),
                                     'shipping_cost' => (int)$order->info['shipping_cost'],
                                     'subtotal' => (int)$order->info['subtotal'],
                                     'total' => (int)$order->info['total']
                                     );
        zen_db_perform(TABLE_REPETITION_CHECK, $repetition_array);
    }
  }
}


if( isset( $_SESSION['shipping']['timespec'] ) && $_SESSION['shipping'] != 'free_free' ){
