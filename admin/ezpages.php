...
// Sets the status of a page
  ...
  // zen-cart.cn - section inserted for logical page headings
  ...
  if (zen_not_null($action)) {
    switch ($action) {
      ...
      case 'insert':
      case 'update':
// zen-cart.cn - start modification for multi-language support
        ...
        if ($zv_link_method_cnt > 1) {
          $messageStack->add(ERROR_MULTIPLE_HTML_URL . '( link_method_cnt = ' . $zv_link_method_cnt . ', pages_html_flag = ' . ($pages_html_flag ? 'true' : 'false') . ')' , 'error');
          $page_error = true;
        }
...
//                $oFCKeditor->Create() ;
        ...
        echo zen_draw_textarea_field('pages_html_text[' . $languages[$i]['id'] . ']', 'soft', '100%', '40', $pages_html_text);
      }
    }
// end modified code for multi-language support
?>
