...
<?php
      ...
      case 'update':
        ...
        $customers_telephone = mb_convert_kana($customers_telephone, "a");
        $customers_fax = mb_convert_kana($customers_fax, "a");
        ...
        $entry_firstname_kana = zen_db_prepare_input($_POST['entry_firstname_kana']);
        $entry_lastname_kana = zen_db_prepare_input($_POST['entry_lastname_kana']);
/*
        if (strlen($entry_firstname_kana) < ENTRY_FIRST_NAME_MIN_LENGTH) {
          $error = true;
          $entry_firstname_kana_error = true;
        } else {
          $entry_firstname_kana_error = false;
        }

        if (strlen($entry_lastname_kana) < ENTRY_LAST_NAME_MIN_LENGTH) {
          $error = true;
          $entry_lastname_kana_error = true;
        } else {
          $entry_lastname_kana_error = false;
        }
*/
        if (ACCOUNT_DOB == 'true') {
      ...
      if ($error == false) {
        ...
        $kana_array = array('entry_firstname_kana' => $entry_firstname_kana,
                                  'entry_lastname_kana' => $entry_lastname_kana);
        $sql_data_array = array_merge($sql_data_array, $kana_array);
      ...
      default:
        $customers = $db->Execute("select c.customers_id, c.customers_gender, c.customers_firstname,
                                          ...
                                          a.entry_firstname_kana, a.entry_lastname_kana
...
<?php
  if ($error == true) {
    if ($entry_firstname_kana_error == true) {
      echo zen_draw_input_field('entry_firstname_kana', $cInfo->entry_firstname_kana, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_firstname_kana', 50)) . '&nbsp;' . ENTRY_FIRST_NAME_KANA_ERROR;
    } else {
      echo $cInfo->entry_firstname_kana . zen_draw_hidden_field('entry_firstname_kana');
    }
  } else {
    echo zen_draw_input_field('entry_firstname_kana', $cInfo->entry_firstname_kana, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_firstname_kana', 50), false);
  }
?></td>
          </tr>
          <tr>
...
<?php
  if ($error == true) {
    if ($entry_lastname_kana_error == true) {
      echo zen_draw_input_field('entry_lastname_kana', $cInfo->entry_lastname_kana, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_lastname_kana', 50)) . '&nbsp;' . ENTRY_LAST_NAME_KANA_ERROR;
    } else {
      echo $cInfo->entry_lastname_kana . zen_draw_hidden_field('entry_lastname_kana');
    }
  } else {
    echo zen_draw_input_field('entry_lastname_kana', $cInfo->entry_lastname_kana, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_lastname_kana', 50), false);
  }
?></td>
          </tr>
<?php
      ...
      if ($entry_state_error == true) {
        if ($entry_state_has_zones == true) {
          ...
          $zones_values = $db->Execute("select zone_name
                                        ...
                                        order by zone_id");
            $zones_array[] = array('id' => '', 'text' => JS_STATE_SELECT);
