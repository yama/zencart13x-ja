 ...
 * @version $Id: enable_error_logging.php 429 2010-12-07 00:15:26Z bigmouse-sugimoto $
 *
 * phpのログをキャッシュディレクトリに保存する設定です.
 * BM_ERROR_LOG_ADMIN_STATUS　をtrueに設定すればphpでエラーが発生した時にはキャッシュディレクトリにエラーを書き出します。
 * webページにエラーを出さずにエラーが発生しているかどうかをチェックできます。
 * ただし、error_reportingの設定によっては細かいエラーも記録されるようになるのでキャッシュディレクトリに大量のファイルが作成されてレスポンスが
 * 悪くなる可能性も有りますので通常運用時にはfalseに設定しておくことをお勧めします。
 * ※ここでの設定は管理画面のみ有効です。
 */

define('BM_ERROR_LOG_ADMIN_STATUS', 'false');

if (BM_ERROR_LOG_ADMIN_STATUS == 'true') {
