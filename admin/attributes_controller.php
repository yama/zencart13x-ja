 ...
 * @version $Id: base.php 1 2007-11-24 20:54:50Z bigmouse $
**/
...
var select1 = document.forms.attributes.options_id;
var select2 = document.forms.attributes.values_id;
select2.options.length = 0;

<?php
    $options_values = $db->Execute("select products_options_name, products_options_id from " . TABLE_PRODUCTS_OPTIONS . "
                                    where language_id = '" . $_SESSION['languages_id'] . "'
                                    order by products_options_name");

$is = 0;
while (!$options_values->EOF) {
    $values_values = $db->Execute("select povtpo.products_options_values_to_products_options_id,
                                    pov.products_options_values_name, povtpo.products_options_values_id
                                   ...
                                   and povtpo.products_options_id = '" . $options_values->fields['products_options_id'] . "'
                                   order by products_options_values_name");
      while (!$values_values->EOF) {
          if ($is == '0') {
            $display_select = 'if (select1.options[select1.selectedIndex].value == "' . $options_values->fields['products_options_id'] . '") { ' . "\n";
          } else {
            $display_select .= 'else if (select1.options[select1.selectedIndex].value == "' . $options_values->fields['products_options_id'] . '") { ' . "\n";
          }
          $iq = 0;
          while (!$values_values->EOF) {
          	$display_select .= 'select2.options[' . $iq . '] = new Option("' . $values_values->fields['products_options_values_name'] . '", "' . $values_values->fields['products_options_values_id'] . '");' . "\n";
          	$iq++;
          	$values_values->MoveNext();
          }
      }
  if ($values_values->RecordCount() > 0) {
    $is++;
    $display_select .= ' }';
  }
...
?>
      ...
      <?php  echo zen_draw_hidden_field('securityToken', $_SESSION['securityToken']); ?><table border="0" cellspacing="0" cellpadding="2">
      ...
      <table border='0' width="100%">
        <tr><td class="attributeBoxContent"><table border="0" cellpadding="4" cellspacing="2">
          ...
          <tr class="attributeBoxContent"><td><table><tr>
            ...
            <td class="attributeBoxContent">&nbsp;<?php echo TABLE_HEADING_OPT_NAME . '<br />'; ?><select name="options_id" size="<?php echo ($action != 'delete_attribute' ? "5" : "1"); ?>" onChange="functioncategory()">
<?php
...
<script type="text/javascript">
<!--
document.write("<select name = \"values_id\" size=\"5\"></select>");
// -->
</script>
<noscript>
<select name="values_id" size="<?php echo ($action != 'delete_attribute' ? "5" : "1"); ?>">
<?php
